 ...
 *
 * goes in webapps/portal/scripts/neoscript.js
 *
 */
            ...
            jQuery.ajax({
                ...
                success: function(data){
                    $.each(data, function(i, item){
                        if (i <= maxToolsInt) {
                            if (item.tools.length === 1) {
                                navsubmenu = navsubmenu + '<li class=\"submenuitem\" ><span><a role=\"menuitem\" href=\"' + item.tools[0].url + "\" title=\"" + item.title + "\"> <i class=\" fa icon-" + item.tools[0].toolId.replace(/\./gi, '-') + "\"></i> " + item.title + "</a></span></li>";
                            }
                        }
                    });
                    if ((data.length - 1) > maxToolsInt) {
